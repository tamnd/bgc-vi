...
#   ./scripts/release.sh --tag v0.10.5-vi.1  # force full tag
...
if [[ -n "$FORCE_TAG" ]]; then
    ...
    if [[ -n "$FORCE_REV" ]]; then
        ...
        # Highest existing -vi.N for this upstream version, plus 1.
...
Vietnamese translation based on upstream beejjorgensen/bgc@$UPSTREAM_COMMIT."
